 ...
 */
public class nurseResController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public nurseResController() {
        super();
	...
	private String insertNote(HttpServletRequest request) {
		...
		nr.setStuId(studentId);
		nr.setNurseId(nurseId);
		nr.setNote(note);
		
		boolean success = false;
		success = nurseResDB.insertNote(nr);
		
		if (success && doctorId > 0) {
			// nurse referred a doctor for this student
			success = nurseDB.insertNewReferral(studentId, nurseId, doctorId);
		}
		
		return success ? "/EYEPortal/viewScore.jsp"
					   : "/EYEPortal/err.html";
	}
